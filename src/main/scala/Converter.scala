...
object  Converter extends App {
       ...
       if (remList.isEmpty || currCount >= size) acc
        ...
        // handle trailing zeros
          ...
          if (size == 1) "zero"
            ...
            val countNonZero = remList.tail.count((p: Char) => {p != '0'})
            if(countNonZero == 0) acc
            else if(remList.tail.size >= 2 && remList.tail.head == '0' && remList.tail.tail.head == '0'){
               loop(currCount + 3, size, acc, remList.tail.tail.tail)
            }
            else{
        ...
        else if (size > currCount + 1 && (currCount + 1) % 3 == 1 && remList.tail.head == '1') {
          // handle teens
  ...
  println(convert("23400002323".toList))
  println(convert("0505050505050".toList))
  println(convert("1001".toList))
